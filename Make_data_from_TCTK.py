...
def main():
    ...
    jet_property_names = {      # keys are output branch names, values are input branch names
        "fjet_m":              "largeRjet_m_NOSYS",
        "fjet_pt":             "largeRjet_pt_NOSYS",
        "fjet_eta":            "largeRjet_eta",
        "fjet_phi":            "largeRjet_phi",
        "fjet_truth_label":    "largeRjet_truth_label",
        "fjet_Nconst_Charged": "largeRjet_Ntrk_NOSYS", # Count number of LRJet charged UFO objects
        "fjet_FourProngLundNetP1": "largeRjet_LundNetScoreP1_NOSYS",
        "fjet_FourProngLundNetP2": "largeRjet_LundNetScoreP2_NOSYS",
        "fjet_FourProngLundNetP3": "largeRjet_LundNetScoreP3_NOSYS",
        "fjet_FourProngLundNetP4": "largeRjet_LundNetScoreP4_NOSYS",
    ...
    for frac_idx in event_fraction_indices:
        ...
        for file_number, file in enumerate(files, start=1):
            ...
            with uproot.open(file) as infile:
                ...
                dsids = tree["mcChannelNumber"].array(library="np")
                ...
                jet_properties = {
                    ...
                    for jet_property in [*jet_property_names.values(), "largeRjet_LundAllZ_NOSYS", "largeRjet_LundAllKt_NOSYS", "largeRjet_LundAllDR_NOSYS", "largeRjet_LundAllIDP1_NOSYS", "largeRjet_LundAllIDP2_NOSYS"]
                    if jet_property in tree
                }
                truth_labels_unflattened = tree["largeRjet_truth_label"].array(entry_start=start_entry, entry_stop=stop_entry, library="ak")
                numbers_of_jets_per_event = ak.num(truth_labels_unflattened)

                mcEventWeights = tree["weight_mc_NOSYS"].array(entry_start=start_entry, entry_stop=stop_entry, library="np")
                ...
                for fjet_weight_pt_branch, s in zip(fjet_weight_pt_branches, signals):
                    jet_properties[fjet_weight_pt_branch] = GetPtWeight(
                        jet_properties["largeRjet_pt_NOSYS"],
                        jet_properties["largeRjet_truth_label"],
                        dsid_test,
                        config_signal[s],
                        SF=5,
                    )
                ...
                dataset = create_train_dataset_fulld_new_Ntrk_pt_weight_file(
                    dataset,
                    *itemgetter("largeRjet_LundAllZ_NOSYS", "largeRjet_LundAllKt_NOSYS", "largeRjet_LundAllDR_NOSYS", "largeRjet_LundAllIDP1_NOSYS", "largeRjet_LundAllIDP2_NOSYS")(jet_properties),
                    *itemgetter(jet_label_branch, "EventInfo_mcChannelNumber", "largeRjet_Ntrk_NOSYS", "largeRjet_pt_NOSYS", "largeRjet_m_NOSYS", "largeRjet_eta")(jet_properties),
