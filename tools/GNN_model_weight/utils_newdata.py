...
def GetPtWeight_all_MC( dsid , signal_config, dsid_input, pt, SF, Pythia_or_All=False ):
    ## PT histograms of all qcd and top jets in dataset
    filename1 = signal_config["pt_hist_file_bkg"]["files"]
    filename2 = signal_config["pt_hist_file_signal"]["files"]
...
def get_scores_manual(loader, model, device): # For regular LundNet
    model.eval()
    total_output = np.array([[1]])
    batch_counter = 0
    for data in loader:
...
def get_scores_manual(loader, model, device): # For ManualLundNet
    model.eval()
    total_output = np.array([[1]])
    seq2 = []
    batch_counter = 0
