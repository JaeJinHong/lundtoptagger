...
def create_train_dataset_fulld_new_Ntrk_pt_weight_file(graphs, z, k, d, edge1, edge2, weight, label, Ntracks, jet_pts, jet_ms, kT_selection, primary_Lund_only_one_arr, GN2X_pqcd, GN2X_phbb, GN2X_ptop, GN2X_phcc, dsid, signal_jet_truth_label):
    ...
    for i in range(len(z)):  
        ...
        graphs.append(Data(x= vec.detach() ,
                           ...
                           GN2X_ptop= float(GN2X_ptop[i]) , GN2X_phcc= float(GN2X_phcc[i]) )) #torch.tensor(label_out, dtype=torch.float).detach() ))
        '''
...
def create_train_dataset_fulld_new_Ntrk_pt_weight_file_test(graphs, graph_small_example, z, k, d, edge1, edge2, weight, label, Ntracks, jet_pts, jet_ms, kT_selection, mcweights, mcweights_out, Good_jets, GN2X_pqcd, GN2X_phbb, GN2X_ptop, GN2X_phcc, signal_jet_truth_label):
