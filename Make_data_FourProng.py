...
def main():
    ...
    mass_windows = config["mass_windows"]  # List of [min, max] pairs
    if mass_windows is None or len(mass_windows) == 0:
        mass_windows = config_signal[signal]["mass_range"]
    print(f"Using mass windows: ", mass_windows)
    ...
    for mass_window in mass_windows: # loop over the mass windows
        ...
        for frac_idx in event_fraction_indices:
                ...
                with uproot.open(file) as infile:
                    ...
                    for fjet_weight_pt_branch, s in zip(fjet_weight_pt_branches, signals):
                        jet_properties[fjet_weight_pt_branch] = GetPtWeight(
                            ...
                            jet_properties[jet_label_branch], # JJ: Able to decide what decoration to select signal
                            dsid_test,
                            config_signal[s],
                            SF=5,
                        )
            ...
            filepath_placeholder_vals = dict(
                ...
                window_low = min(mass_window),
                window_high = max(mass_window),
            )
